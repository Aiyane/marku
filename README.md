...
这个项目现在已经改成Web项目, 由于前期的设计失误, 在渲染阶段进行了两次不同的渲染, 分别处理左边的文字框和右边的结果框. 右边是即时渲染, 检测左边有任何改动则渲染. 左边的是按键渲染, 在按下Tab键之后会渲染文字, 以下为示例图片:

![](http://onqow625k.bkt.clouddn.com/photo.png "图片")
