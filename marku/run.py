...
class Marku(HTMLRenderer):
    def __init__(self, input_file, css=None, other='', highlight=True, txt_render=False):
        """
        input_file: 输入文件路径
        css_file: 输入css文件路径
        other: head标签的其他语句
    ...
    def render(self, output_file=None):
        ...
        rendered = self.rendered(AST, self.css, self.other, self.highlight, self.txt_render)
...
class Mark(TxtRender):
    ...
    def _add_tokens(self, tokens):
        ...
        增加额外的tokens
        """
        super(Mark, self).__init__(*tokens)
        for token in tokens:
            if hasattr(token, 'match'):
                big_token.add_token(token)
            else:
